...
def make_suite(module_names, position='at_install'):
    """ Creates a tests suite for all the tests in the specified modules,
    filtered by the provided ``position`` and the current tests tags
...
def unwrap_suite(test):
    ...
    generate a single stream of terminals (either tests cases or customized
    tests suites). These can then be checked for run/skip attributes
    individually.
    ...
    subtests = list(test)
    ## custom tests suite (no tests cases)
    #if not len(subtests):
    #    yield tests
    #    return
