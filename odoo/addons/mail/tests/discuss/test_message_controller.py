...
class TestMessageController(HttpCaseWithUserDemo):
    ...
    def test_channel_message_attachments(self):
        ...
        # tests message post: token error
        res1 = self.url_open(
        ...
        # tests message update: token error
        res3 = self.url_open(
        ...
        # tests message update: own attachment ok
        res5 = self.url_open(
            url="/mail/message/update_content",
            data=json.dumps(
                {
    ...
    def test_attachment_hijack(self):
        ...
        )  # if this tests breaks, it might be due to a change in /web/content, or the default rules for accessing an attachment. This is not an issue but it makes this tests irrelevant.
    ...
    def test_mail_partner_from_email_authenticated(self):
        ...
        self.assertEqual(
            0,
            self.env["res.partner"].search_count([('email', '=', "john@tests.be")]),
            ...
            data=json.dumps(
                {
                    "params": {
                        ...
                        "thread_id": self.channel.id,
                        "emails": ["john@tests.be"],
                        'additional_values': {"john@tests.be": {'phone': '123456789'}},
                    },
                }
            ),
            headers={"Content-Type": "application/json"},
        ...
        self.assertEqual(
            1,
            self.env["res.partner"].search_count([('email', '=', "john@tests.be"), ('phone', '=', "123456789")]),
            ...
            data=json.dumps(
                {
                    "params": {
                        ...
                        "post_data": {
                            "body": "tests",
                            "partner_emails": ["john2@tests.be"],
                            "partner_additional_values": {"john2@tests.be": {'phone': '123456789'}},
                        },
                    },
                }
            ),
        ...
        self.assertEqual(
            1,
            self.env["res.partner"].search_count([('email', '=', "john2@tests.be"), ('phone', '=', "123456789")]),
        ...
        self.assertEqual(
            1,
            self.env["res.partner"].search_count([('email', '=', "john2@tests.be")]),
