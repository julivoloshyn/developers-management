...
class TestMailTools(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls._test_email = 'alfredoastaire@tests.example.com'
    ...
    def test_find_partner_from_emails(self):
        ...
        sources = [
            self._test_email,  # tests direct match
            f'"Norbert Poiluchette" <{self._test_email}>',  # encapsulated
            'fredoastaire@tests.example.com',  # partial email -> should not match !
        ]
        expected_partners = [
            test_partner,
            test_partner,
        ...
        # tests with wildcard "_"
        found = Partner._mail_find_partner_from_emails(['alfred_astaire@tests.example.com'])
        self.assertEqual(found, [self.env['res.partner']])
        # sub-check: this search does not consider _ as a wildcard
        found = Partner._mail_search_on_partner(['alfred_astaire@tests.example.com'])
        self.assertEqual(found, self.env['res.partner'])

        # tests partners with encapsulated emails
        # ------------------------------------------------------------
    ...
    def test_mail_find_partner_from_emails_followers(self):
        ...
        # standard tests, no multi-email, to assert base behavior
        ...
        # multi-email
        _test_email_2 = '"Robert Astaire" <not.alfredoastaire@tests.example.com>'
        ...
        sources = [
            ...
            ('not.alfredoastaire@tests.example.com', True),  # normalized second email in field
            ('not.alfredoastaire@tests.example.com', False),  # normalized second email in field
        ...
        # tests users with same email, priority given to current user
        # --------------------------------------------------------------
...
class TestMailUtils(MailCommon):
    ...
    def test_migrate_icp_to_domain(self):
        ...
        self.env["ir.config_parameter"].set_param("mail.catchall.domain", "tests.migration.com")
        ...
        existing = self.env["mail.alias.domain"].search([('name', '=', 'tests.migration.com')])
        self.assertFalse(existing)

        new = self.env["mail.alias.domain"]._migrate_icp_to_domain()
        self.assertEqual(new.name, "tests.migration.com")
        ...
        self.assertEqual(again.name, "tests.migration.com")
