...
class TestWebsiteVisitor(TestCrmCommon, WebsiteVisitorTests):
    ...
    def setUp(self):
        ...
        self.test_partner = self.env['res.partner'].create({
            'name': 'Test Customer',
            'email': '"Test Customer" <tests@tests.example.com>',
    ...
    def test_compute_email_phone(self):
        ...
        lead_1 = self.env['crm.lead'].create({
            'name': 'Test Lead 1',
            'email_from': 'Rambeau Fort <beaufort@tests.example.com',
        ...
        lead_2 = self.env['crm.lead'].create({
            'name': 'Test Lead 1',
            'email_from': 'Martino Brie <brie@tests.example.com',
            'country_id': self.env.ref('base.be').id,
            'mobile': '+32456001122',
            'visitor_ids': [(4, visitor.id)],
        })
    ...
    def test_link_to_visitor_crm(self):
        """ Same as parent's 'test_link_to_visitor' except we also tests that leads
