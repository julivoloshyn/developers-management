...
class TestPoSSale(TestPointOfSaleHttpCommon):
    def test_settle_order_with_kit(self):
        if not self.env["ir.module.module"].search([("name", "=", "mrp"), ("state", "=", "installed")]):
            self.skipTest("mrp module is required for this tests")
    ...
    def test_settle_order_with_different_product(self):
        """This tests create an order and settle it in the PoS. But only one of the product is delivered.
    ...
    def test_settle_order_with_multistep_delivery(self):
        """This tests create an order and settle it in the PoS. It also uses multistep delivery
    ...
    def test_pos_invoice_analytic_account(self):
        ...
        self.analytic_plan_departments = self.env['account.analytic.plan'].create({'name': 'Departments tests'})
