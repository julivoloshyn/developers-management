...
class TestSMSPost(SMSCommon, TestSMSRecipients, CronMixinCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_record = cls.env['mail.tests.sms'].with_context(**cls._test_context).create({
            'name': 'Test',
    ...
    def test_message_sms_internals_body(self):
        with self.with_user('employee'), self.mockSMSGateway():
            test_record = self.env['mail.tests.sms'].browse(self.test_record.id)
    ...
    def test_message_sms_internals_sms_numbers_duplicate(self):
        """ _message_sms ( which uses _notify_thread_by_sms) allows for specifying additional number to send sms to
            This tests checks for situation where this additional number is the same as partner telephone number.
    ...
    def test_message_sms_model_w_partner_only(self):
        with self.with_user('employee'):
            record = self.env['mail.tests.sms.partner'].create({'customer_id': self.partner_1.id})
    ...
    def test_message_sms_model_w_partner_only_void(self):
        with self.with_user('employee'):
            record = self.env['mail.tests.sms.partner'].create({'customer_id': False})
    ...
    def test_message_sms_model_w_partner_m2m_only(self):
        with self.with_user('employee'):
            record = self.env['mail.tests.sms.partner.2many'].create({'customer_ids': [(4, self.partner_1.id)]})
        ...
        with self.with_user('employee'):
            record = self.env['mail.tests.sms.partner.2many'].create({'customer_ids': [(4, self.partner_1.id), (4, self.partner_2.id)]})
    ...
    def test_message_sms_with_template(self):
        sms_template = self.env['sms.template'].create({
            'name': 'Test Template',
            'model_id': self.env['ir.model']._get('mail.tests.sms').id,
...
class TestSMSApi(SMSCommon):
    ...
    def setUpClass(cls):
        ...
        cls._create_records_for_batch('mail.tests.sms', 3)
        cls.sms_template = cls._create_sms_template('mail.tests.sms')

    def test_message_schedule_sms(self):
        with self.with_user('employee'):
            with self.mockSMSGateway():
                self.env['mail.tests.sms'].browse(self.records.ids)._message_sms_schedule_mass(body=self._test_body, mass_keep_log=False)
    ...
    def test_message_schedule_sms_w_log(self):
        with self.with_user('employee'):
            with self.mockSMSGateway():
                self.env['mail.tests.sms'].browse(self.records.ids)._message_sms_schedule_mass(body=self._test_body, mass_keep_log=True)
    ...
    def test_message_schedule_sms_w_template(self):
        with self.with_user('employee'):
            with self.mockSMSGateway():
                self.env['mail.tests.sms'].browse(self.records.ids)._message_sms_schedule_mass(template=self.sms_template, mass_keep_log=False)
    ...
    def test_message_schedule_sms_w_template_and_log(self):
        with self.with_user('employee'):
            with self.mockSMSGateway():
                self.env['mail.tests.sms'].browse(self.records.ids)._message_sms_schedule_mass(template=self.sms_template, mass_keep_log=True)
