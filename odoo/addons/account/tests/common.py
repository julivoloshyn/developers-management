...
def instantiate_accountman(cls):
    ...
    cls.user.partner_id.email = 'accountman@tests.com'

    # Shadow the current environment/cursor with one having the report user.
    # This is mandatory to tests access rights.
...
class AccountTestInvoicingCommon(TransactionCase):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        assert 'post_install' in cls.test_tags, 'This tests requires a CoA to be installed, it should be tagged "post_install"'
        ...
        if chart_template_ref:
            ...
            if template_module.state != 'installed':
                raise SkipTest(f"Module required for the tests is not installed ({template_module.name})")
