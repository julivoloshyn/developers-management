...
class TestThirdChecks(L10nLatamCheckTest):
    ...
    def test_01_get_paid_with_multiple_checks(self):
        """ This a generic tests to check that we are able to pay with checks
    ...
    def test_04_check_transfer(self):
        ...
        # Transfer to rejected checks journal (usually is to another third party checks journal, but for tests purpose is the same)
        self.env['l10n_latam.payment.mass.transfer'].with_context(
        ...
        # tests that checks created on different journals but that are on same current journal, can be transfered together
