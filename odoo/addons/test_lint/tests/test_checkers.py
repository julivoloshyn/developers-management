...
class TestSqlLint(TransactionCase):
    ...
    def test_sql_injection_detection(self):
        ...
        self.linter.current_file = 'dummy.py' # should not be prefixed by tests
        ...
        def test_function4(self, arg):
            my_injection_variable= "aaaaaaaa {tests}".format(tests="aaa") 
            self.env.cr.execute('select * from hello where id = %s' % my_injection_variable) #@
        """)
        with self.assertNoMessages():
            checker.visit_call(node)
        ...
        def test_function5(self):
            arg = 'aaa'
            my_injection_variable= "aaaaaaaa {tests}".format(tests=arg) #Uninferable
            self.env.cr.execute('select * from hello where id = %s' % my_injection_variable) #@
        """)
        with self.assertNoMessages():
            checker.visit_call(node)
        ...
        def test_function6(self,arg):
            my_injection_variable= "aaaaaaaa {tests}".format(tests="aaa" + arg) #Uninferable
            self.env.cr.execute('select * from hello where id = %s' % my_injection_variable) #@
        """)
        with self.assertMessages("sql-injection"):
            checker.visit_call(node)
        ...
        def test_function7(self):
            arg = "aaa"
            my_injection_variable= "aaaaaaaa {tests}".format(tests="aaa" + arg) #Const
            self.env.cr.execute('select * from hello where id = %s' % my_injection_variable)#@
        """)
        with self.assertNoMessages():
            checker.visit_call(node)
        ...
        #    def tests():
        #        return "hello world"
        #    my_injection_variable= "aaaaaaaa {tests}".format(tests=tests()) #Const
        #    self.env.cr.execute('select * from hello where id = %s' % my_injection_variable) #@
        #""")
        #with self.assertNoMessages():
        #    checker.visit_call(node)
