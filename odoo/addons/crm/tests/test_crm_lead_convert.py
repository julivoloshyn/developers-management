...
class TestLeadConvertForm(crm_common.TestLeadConvertCommon):
    ...
    def test_form_action_default(self):
        ...
        customer = self.env['res.partner'].create({
            "name": "Amy Wong",
            "email": '"Amy, PhD Student, Wong" Tiny <AMY.WONG@tests.example.com>'
        })
...
class TestLeadConvert(crm_common.TestLeadConvertCommon):
    ...
    def test_initial_data(self):
        """ Ensure initial data to avoid spaghetti tests update afterwards """
    ...
    def test_lead_convert_base(self):
        ...
        self.assertEqual(lead.email_from, 'amy.wong@tests.example.com')
    ...
    def test_lead_convert_corner_cases_matching(self):
        ...
        # email formatting
        self.lead_1.write({
            'email_from': 'Amy Wong <amy.wong@tests.example.com>'
        })
        customer = self.env['res.partner'].create({
            'name': 'Different Name',
            'email': 'Wong AMY <AMY.WONG@tests.example.com>'
        })
    ...
    def test_lead_convert_internals(self):
        ...
        # tests internals of convert wizard
        ...
        self.assertEqual(new_partner.email, 'amy.wong@tests.example.com')
    ...
    def test_lead_convert_contact_mutlicompany(self):
        ...
        partner_company_2 = self.env['res.partner'].with_user(SUPERUSER_ID).create({
            ...
            'email': 'tests@company2.com',
            'company_id': company_2.id,
        })
        lead = self.env['crm.lead'].create({
            'name': 'LEAD',
            'type': 'lead',
            'email_from': 'tests@company2.com',
        })
        convert = self.env['crm.lead2opportunity.partner'].with_context({
            'active_model': 'crm.lead',
            'active_id': lead.id,
    ...
    def test_lead_convert_same_partner(self):
        ...
        lead = self.env['crm.lead'].create({
            ...
            'type': 'lead',
            'email_from': 'demo@tests.com',
            'lang_id': self.lang_fr.id,
            'street': 'my street',
            'city': 'my city',
        })
        lead.convert_opportunity(partner)
        self.assertEqual(lead.email_from, 'demo@tests.com', 'Email From should be preserved during conversion')
    ...
    def test_lead_convert_properties_preserve(self):
        ...
        self.assertEqual(self.lead_1.lead_properties, {'tests': 'tests value'})
