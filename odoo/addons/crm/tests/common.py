...
class TestCrmCommon(TestSalesCommon, MailCase):
    ...
    def setUpClass(cls):
        ...
        cls.env['crm.stage'].search([]).write({'sequence': 9999})  # ensure search will find tests data first
        ...
        cls.lead_1 = cls.env['crm.lead'].create({
            ...
            'email_from': 'amy.wong@tests.example.com',
        ...
        # email / phone data
        cls.test_email_data = [
            '"Planet Express" <planet.express@tests.example.com>',
            '"Philip, J. Fry" <philip.j.fry@tests.example.com>',
            '"Turanga Leela" <turanga.leela@tests.example.com>',
        ]
        cls.test_email_data_normalized = [
            'planet.express@tests.example.com',
            'philip.j.fry@tests.example.com',
            'turanga.leela@tests.example.com',
        ]
        cls.test_phone_data = [
        ...
        # create some tests contact and companies
    ...
    def _activate_multi_company(cls):
        ...
        cls.user_sales_manager_mc = mail_new_test_user(
            ...
            email='user.sales.manager.mc@tests.example.com',
    ...
    def _create_duplicates(self, lead, create_opp=True):
        """ Helper tool method creating, based on a given lead
          ...
          * a customer (res.partner) based on lead email (to tests partner finding)
        ...
        try:
            ...
            for fname in fields_all:
                ...
                if fname == 'description':
                    ...
                    # Specific computation, has its own tests
                    continue
                else:
                    self.assertEqual(
...
class TestLeadConvertMassCommon(TestLeadConvertCommon):
    ...
    def setUpClass(cls):
        ...
        cls.user_sales_leads_convert = mail_new_test_user(
            ...
            name='Lucien Sales Leads Convert', email='crm_leads_2@tests.example.com',
        ...
        cls.lead_w_partner_company = cls.env['crm.lead'].create({
            ...
            'email_from': 'hermes.conrad@tests.example.com',
        ...
        cls.lead_w_email = cls.env['crm.lead'].create({
            ...
            'priority': '2',
            'probability': 15,
            'email_from': 'contact.email@tests.example.com',
            'user_id': cls.user_sales_salesman.id,
            'stage_id': cls.stage_gen_1.id,
        })
        cls.lead_w_email_lost = cls.env['crm.lead'].create({
            ...
            'type': 'lead',
            'probability': 15,
            'email_from': 'strange.from@tests.example.com',
            'user_id': cls.user_sales_leads.id,
