...
class TestActivityCommon(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.test_record, cls.test_record_2 = cls.env['mail.tests.activity'].with_context(cls._test_context).create([
...
class TestActivityRights(TestActivityCommon):
    ...
    def test_activity_security_user_noaccess_automated(self):
        def _employee_crash(*args, **kwargs):
            """ If employee is tests employee, consider they have no access on document """
            recordset = args[0]
...
class TestActivityFlow(TestActivityCommon):
    ...
    def test_activity_flow_employee(self):
        with self.with_user('employee'):
            test_record = self.env['mail.tests.activity'].browse(self.test_record.id)
    ...
    def test_activity_summary_sync(self):
        ...
        with Form(self.env['mail.activity'].with_context(default_res_model_id=self.env['ir.model']._get_id('mail.tests.activity'), default_res_id=self.test_record.id)) as ActivityForm:
    ...
    def test_activity_values(self):
        ...
        temp_record = self.env['mail.tests.activity'].new({'name': 'Test'})
        with self.assertRaises(IntegrityError):
            activity = temp_record.activity_schedule('test_mail.mail_act_test_todo', user_id=self.user_employee.id)

        test_record = self.env['mail.tests.activity'].browse(self.test_record.ids)
...
class TestActivityMixin(TestActivityCommon):
    ...
    def test_activity_mixin(self):
        ...
        with self.with_user('employee'):
            self.test_record = self.env['mail.tests.activity'].browse(self.test_record.id)
    ...
    def test_feedback_w_attachments(self):
        ...
        activity = self.env['mail.activity'].create({
            'activity_type_id': 1,
            'res_id': test_record.id,
            'res_model_id': self.env['ir.model']._get_id('mail.tests.activity'),
            'summary': 'Test',
        })
    ...
    def test_mail_activity_state(self):
        ...
        record = self.env['mail.tests.activity'].create({'name': 'Record'})
    ...
    def test_mail_activity_mixin_search_activity_user_id_false(self):
        ...
        MailTestActivity = self.env['mail.tests.activity']
    ...
    def test_mail_activity_mixin_search_state_basic(self):
        ...
        Test all the operators and also tests the case where the "activity_state" is
        ...
        self.env['mail.tests.activity'].create([
            {'name': 'Record %i' % record_i}
            for record_i in range(5)
        ])

        origin_1, origin_2 = self.env['mail.tests.activity'].search([], limit=2)
        ...
        with patch('odoo.addons.mail.models.mail_activity.datetime', MockedDatetime), \
            ...
            all_activity_mixin_record = self.env['mail.tests.activity'].search([])

            result = self.env['mail.tests.activity'].search([('activity_state', '=', 'today')])
            self.assertTrue(len(result) > 0)
            self.assertEqual(result, all_activity_mixin_record.filtered(lambda p: p.activity_state == 'today'))

            result = self.env['mail.tests.activity'].search([('activity_state', 'in', ('today', 'overdue'))])
            self.assertTrue(len(result) > 0)
            self.assertEqual(result, all_activity_mixin_record.filtered(lambda p: p.activity_state in ('today', 'overdue')))

            result = self.env['mail.tests.activity'].search([('activity_state', 'not in', ('today',))])
            self.assertTrue(len(result) > 0)
            self.assertEqual(result, all_activity_mixin_record.filtered(lambda p: p.activity_state not in ('today',)))

            result = self.env['mail.tests.activity'].search([('activity_state', '=', False)])
            ...
            result = self.env['mail.tests.activity'].search([('activity_state', 'not in', ('planned', 'overdue', 'today'))])
            self.assertTrue(len(result) >= 3, "There is more than 3 records without an activity schedule on it")
            self.assertEqual(result, all_activity_mixin_record.filtered(lambda p: not p.activity_state))

            # tests tricky case when the domain will be reversed in the search method
            # because of falsy value
            result = self.env['mail.tests.activity'].search([('activity_state', 'not in', ('today', False))])
            self.assertTrue(len(result) > 0)
            self.assertEqual(result, all_activity_mixin_record.filtered(lambda p: p.activity_state not in ('today', False)))

            result = self.env['mail.tests.activity'].search([('activity_state', 'in', ('today', False))])
            self.assertTrue(len(result) > 0)
            ...
            Model = self.env['mail.tests.activity']
            search_params = {
    ...
    def test_mail_activity_mixin_search_state_different_day_but_close_time(self):
        ...
        that's why it has its own tests.
        """
        ...
        origin_1 = self.env['mail.tests.activity'].search([], limit=1)
    ...
    def test_my_activity_flow_employee(self):
        ...
        test_record_1 = self.env['mail.tests.activity'].with_context(self._test_context).create({'name': 'Test 1'})
        Activity.create({
            'activity_type_id': self.env.ref('test_mail.mail_act_test_todo').id,
            'date_deadline': date_today,
            'res_model_id': self.env.ref('test_mail.model_mail_test_activity').id,
        ...
        with self.with_user('employee'):
            record = self.env['mail.tests.activity'].search([('my_activity_date_deadline', '=', date_today)])
...
class TestActivityViewHelpers(TestActivityCommon):
    ...
    def test_get_activity_data(self):
        ...
        with self.with_user('employee'):
            ...
            test_record, test_record_2 = self.env['mail.tests.activity'].browse(
            ...
            activity_data = get_activity_data('mail.tests.activity', None, fetch_done=True)
            ...
            self.assertEqual(
                get_activity_data('mail.tests.activity', domain=[('id', 'in', test_record.ids)],
                                  fetch_done=True)['activity_res_ids'],
                [test_record.id])
            self.assertEqual(get_activity_data('mail.tests.activity', None, fetch_done=False)['activity_res_ids'],
                             [test_record_2.id])
            # Note that the records are ordered by ids not by deadline (so we get the "wrong" order)
            self.assertEqual(
                get_activity_data('mail.tests.activity', None, offset=1, fetch_done=True)['activity_res_ids'],
                [test_record_2.id])
            self.assertEqual(
                get_activity_data('mail.tests.activity', None, limit=1, fetch_done=True)['activity_res_ids'],
                [test_record.id])
...
class TestORM(TestActivityCommon):
    ...
    def test_week_grouping(self):
        ...
        """
        MailTestActivityCtx = self.env['mail.tests.activity'].with_context({"lang": "en_US"})
        ...
        self.env['mail.tests.activity'].create({
            ...
            'test_mail.mail_act_test_todo',
            summary="Make another tests super asap (yesterday)",
        ...
        self.env['mail.tests.activity'].create({
            ...
            'test_mail.mail_act_test_todo',
            summary="Make another tests asap",
...
class TestTours(HttpCase):
    def test_activity_view_data_with_offset(self):
        ...
        MailTestActivityModel = self.env['mail.tests.activity']
