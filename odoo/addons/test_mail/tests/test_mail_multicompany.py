...
class TestMailMCCommon(MailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.test_model = cls.env['ir.model']._get('mail.tests.gateway')
        cls.email_from = '"Sylvie Lelitre" <tests.sylvie.lelitre@agrolait.com>'

        cls.test_record = cls.env['mail.tests.gateway'].with_context(cls._test_context).create({
            'name': 'Test',
            'email_from': 'ignasse@example.com',
        }).with_context({})
        cls.test_records_mc = cls.env['mail.tests.multi.company'].create([
            {'name': 'Test Company1',
        ...
        # groups@.. will cause the creation of new mail.tests.gateway
        ...
        # Set a first message on public group to tests update and hierarchy
        cls.fake_email = cls.env['mail.message'].create({
            'model': 'mail.tests.gateway',
            'res_id': cls.test_record.id,
            'subject': 'Public Discussion',
            'message_type': 'email',
            'subtype_id': cls.env.ref('mail.mt_comment').id,
            'author_id': cls.partner_1.id,
            'message_id': '<123456-openerp-%s-mail.tests.gateway@%s>' % (cls.test_record.id, socket.gethostname()),
        })
...
class TestMultiCompanySetup(TestMailMCCommon):
    ...
    def test_post_with_read_access(self):
        ...
        test_record_c1_su = self.env['mail.tests.multi.company.read'].sudo().create([
            {
        ...
        with patch.object(Message, 'check_access_rule',
            ...
            with self.assertRaises(AccessError):
                test_records_mc_c1.message_post(
                    ...
                    reply_to='custom.reply.to@tests.example.com',  # avoid ACL in notify_get_reply_to
                    subtype_xmlid='mail.mt_comment',
                )
    ...
    def test_systray_get_activities(self):
        ...
        test_records = self.env["mail.tests.multi.company.with.activity"].create(
            [
        ...
        test_activity = next(
            ...
            if a['model'] == 'mail.tests.multi.company.with.activity'
        )
        self.assertEqual(
            test_activity,
            {
                "icon": "/base/static/description/icon.png",
                "id": self.env["ir.model"]._get_id("mail.tests.multi.company.with.activity"),
                "model": "mail.tests.multi.company.with.activity",
...
class TestMultiCompanyRedirect(MailCommon, HttpCase):
    ...
    def test_redirect_to_records(self):
        ...
        mc_record_c1, mc_record_c2 = self.env['mail.tests.multi.company'].create([
            {
                'company_id': self.user_employee.company_id.id,
    ...
    def test_redirect_to_records_nothread(self):
        ...
        nothreads = self.env['mail.tests.nothread'].create([
            {
