...
class TestMessageValues(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.alias_record = cls.env['mail.tests.container'].with_context(cls._test_context).create({
            'name': 'Pigs',
            'alias_name': 'pigs',
    ...
    def test_empty_message(self):
        ...
        record = self.env['mail.tests.track'].create({'name': 'EmptyTesting'})
        self.flush_tracking()
    ...
    def test_mail_message_format(self):
        record1 = self.env['mail.tests.simple'].create({'name': 'Test1'})
        record2 = self.env['mail.tests.nothread'].create({'name': 'Test2'})
    ...
    def test_mail_message_format_access(self):
        ...
        record1 = self.env['mail.tests.multi.company'].create({
            'name': 'Test1',
    ...
    def test_mail_message_values_fromto_long_name(self):
        ...
        + commit linked to this tests). """
        # name would make it blow up: keep only email
        test_record = self.env['mail.tests.container'].browse(self.alias_record.ids)
        test_record.write({
    ...
    def test_mail_message_values_fromto_no_document_values(self):
        msg = self.Message.create({
            'reply_to': 'tests.reply@example.com',
            'email_from': 'tests.from@example.com',
        })
        self.assertIn('-private', msg.message_id.split('@')[0], 'mail_message: message_id for a void message should be a "private" one')
        self.assertEqual(msg.reply_to, 'tests.reply@example.com')
        self.assertEqual(msg.email_from, 'tests.from@example.com')
    ...
    def test_mail_message_values_fromto_document_alias(self):
        msg = self.Message.create({
            'model': 'mail.tests.container',
            'res_id': self.alias_record.id
        })
        self.assertIn('-openerp-%d-mail.tests' % self.alias_record.id, msg.message_id.split('@')[0])
    ...
    def test_mail_message_values_fromto_document_no_alias(self):
        test_record = self.env['mail.tests.simple'].create({'name': 'Test', 'email_from': 'ignasse@example.com'})

        msg = self.Message.create({
            'model': 'mail.tests.simple',
            'res_id': test_record.id
        })
        self.assertIn('-openerp-%d-mail.tests.simple' % test_record.id, msg.message_id.split('@')[0])
    ...
    def test_mail_message_values_fromto_document_manual_alias(self):
        ...
        alias = self.env['mail.alias'].create({
            'alias_name': 'MegaLias',
            'alias_model_id': self.env['ir.model']._get('mail.tests.simple').id,
            'alias_parent_model_id': self.env['ir.model']._get('mail.tests.simple').id,
    ...
    def test_mail_message_values_fromto_reply_to_force_new(self):
        ...
        self.assertNotIn('mail.tests.container', msg.message_id.split('@')[0])
...
class TestMessageAccess(MailCommon):
    ...
    def test_mail_message_access_create_wo_parent_access(self):
        """ Purpose is to tests posting a message on a record whose first message / parent
        is not accessible by current user. """
        test_record = self.env['mail.tests.simple'].with_context(self._test_context).create({'name': 'Test', 'email_from': 'ignasse@example.com'})
